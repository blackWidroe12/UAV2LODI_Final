...
export function AppShell() {
  ...
  if (!sessionChecked) {
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-[#0E1117]">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-[#00D4FF] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-sm text-[#9CA3AF]">Verifying session...</p>
        </div>
      </div>
    );
  }
  ...
  const handleOpenProject = (project: ProjectConfig) => {
    usePipelineStore.getState().resetPipeline(); // Project Isolation: Clean Slate for Every New Project or switch
    loadProject(project);
    setCurrentView('cockpit');
  };
